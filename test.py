...
if __name__=='__main__':
    ...
    # debts = debts[['pre_bond_id', 'bond_nm', 'stock_id', 'stock_nm','convert_price_valid_from', 'premium_rt','year_left','orig_iss_amt','force_redeem_price','sprice']]
    ...
    # debts[['orig_iss_amt','year_left','force_redeem_price','sprice']] = debts[['orig_iss_amt','year_left','force_redeem_price','sprice']].apply(
    #     pd.to_numeric, errors='coerce')
    # debts['stock_nm'] = debts.apply(lambda x: '<a href="https://xueqiu.com/S/{symbol}">{stnm}</a>'.format(
    #     symbol=x['stock_id'].upper(), stnm=x['stock_nm']), axis=1)
    # debts['距强赎价比'] = debts.apply(lambda x:x['force_redeem_price']/x['sprice']-1, axis=1)
    # debts.set_index('pre_bond_id', inplace=True)
    # debtsNew = debts[debts['convert_price_valid_from'].isna()].copy().sort_values(by=['距强赎价比'])
    # debts = debtsNew.append(debts[~debts.index.isin(debtsNew.index)].sort_values(by=['year_left'],ascending=False)).drop(columns=['force_redeem_price','sprice'])
