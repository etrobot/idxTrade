...
def cauculate(dfk):
    ...
    mtm_1 = abs(closes[-10:].mean()-closes.mean())/(closes[-20]-closes[-1])
    mtm_2 = (closes[-10:].mean()-closes.mean())/((max(closes[-5:])+min(closes[-5:]))/2-closes[-10:].mean())*vol.argmin()*min(closes[-7:])/closes[-5]
