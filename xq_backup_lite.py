...
def cauculate(dfk):
    ...
    mtm_1 = (closes[-10:].mean()-closes.mean())/abs(closes[-20]-closes[-1])*(closes[-7:].argmin()-closes.argmin())*closes[-1]/closes[-2]
...
def df2md(mkt, calKey, indDf, pdate, test=0, num=10):
    ...
    if mkt=='cn':
        ...
        debts = debtsNew.append(
            debts[~debts.index.isin(debtsNew.index)].sort_values(by=['year_left'], ascending=False)).drop(
