...
def cauculate(dfk):
    ...
    mtm_2 = [closes[i]/min(closes[-15],closes[-1]*(1+max(dfk['percent'][-30:])*2)) - 1 for i in range(-15,0)]
    cal = sum(mtm_1)
    cal2 = sum(mtm_2)-sum(closes[i]/max(closes[-30],closes[-15]) - 1 for i in range(-30,-15))
