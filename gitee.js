...
async function giteeUpdate() {
    ...
    // 2. 填入账号
    await accountElements[0].type('upknow')
    // 3. 选中密码控件
    let pwdElements = await page.$x('//*[@id="user_password"]')
    // 4. 填入密码
    await pwdElements[0].type('ad2020')
    // 5. 点击登录
    let loginButtons = await page.$x('//*[@id="new_user"]/div[2]/div/div/div[4]/input')
    await loginButtons[0].click()
    // 6. 等待登录成功
    await page.waitFor(1000)
    await page.goto('https://gitee.com/upknow/upknow/pages'); // 比如： https://gitee.com/yang0033/hexo-blog/pages
